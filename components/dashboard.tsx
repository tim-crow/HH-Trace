...
export function Dashboard({ inventory, orders, onNavigate }: DashboardProps) {
  ...
  React.useEffect(() => {
    supabase
      ...
      .limit(5)
      .then(({ data }) => {
        if (data) setRecentAudit(data.map((row: any) => ({
          id: row.id, timestamp: row.timestamp, userName: row.user_name,
          userRole: row.user_role, action: row.action, target: row.target, details: row.details,
        })))
      })
  }, [])
